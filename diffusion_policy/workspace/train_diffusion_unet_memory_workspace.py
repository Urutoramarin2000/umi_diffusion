...
class TrainDiffusionUnetMemoryWorkspace(BaseWorkspace):
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.sample_every) == 0 and accelerator.is_main_process:
                    with torch.no_grad():
                        ...
                        pred_action = policy.predict_action(batch, None, False)['action_pred']
...
def main(cfg):
    workspace = TrainDiffusionUnetMemoryWorkspace(cfg)
    workspace.run()
