...
from diffusion_policy.model.vision.timm_obs_memory_encoder import TimmObsmemoryEncoder
from diffusion_policy.common.pytorch_util import dict_apply


class DiffusionUnetTimmMemoryPolicy(BaseImagePolicy):
    def __init__(self, 
            shape_meta: dict,
            noise_scheduler: DDPMScheduler,
            obs_encoder: TimmObsmemoryEncoder,
    ...
    def predict_action(self, batch, fixed_action_prefix: torch.Tensor=None, Implement: bool = False) -> Dict[str, torch.Tensor]:
        """
        ...
        if not Implement:
            ...
            nmemory = self.normalizer.normalize(batch['memory'])
            B = next(iter(nobs.values())).shape[0]

            # condition through global feature
            obs_dict = {"obs": nobs, "mem": nmemory}
