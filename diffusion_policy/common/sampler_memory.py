...
class SequenceSampler:
    ...
    def sample_sequence(self, idx):
        ...
        for key in self.memory_keys:
            input_arr = self.replay_buffer[key]
            this_horizon = self.key_horizon[key]
            # print("this_horizon", this_horizon)
            ...
            for image_key in self.rgb_keys:
                ...
                while i < end_idx:
                    ...
                    # 随机选取(i, i+10)
                    index = np.random.randint(i, min(i + 10, end_idx))
                    output.append(input_image[index])
                    memory_size += 1

                    # 修改这里：不要提前停止（去掉 if memory_size > 2）
                    if i + 11 < end_idx:
                        i += 11
                    else:
                        break
