...
class TransformerObsMemoryEncoder(ModuleAttrMixin):
    def __init__(self,
            shape_meta: dict,
        ...
        memory_shape_meta = shape_meta['memory']
        memory_shape = None
        ...
        for key, attr in memory_shape_meta.items():
            type = attr.get('type', 'low_dim')
            key_shape_map[key] = memory_shape
            if type == 'low_dim':
                ...
                # check if we need feature projection
                with torch.no_grad():
                    example_img = torch.zeros((1,)+tuple(memory_shape))
                    example_feature_map = this_model(example_img)
                    example_features = self.aggregate_feature(example_feature_map)
                    feature_shape = example_features.shape
                    feature_size = feature_shape[-1]
    ...
    def forward(self, obs_dict):
        embeddings = list()
        batch_size = next(iter(obs_dict["obs"].values())).shape[0]
    ...
    def output_shape(self):
        ...
        example_obs_dict = {"obs": {}, "mem": {}}
        obs_shape_meta = self.shape_meta['obs']
        mem_shape_meta = self.shape_meta['memory']
        for key, attr in obs_shape_meta.items():
            shape = tuple(attr['shape'])
            this_obs = torch.zeros(
                ...
                dtype=self.dtype,
                device=self.device)
            example_obs_dict["obs"][key] = this_obs
            if key == "camera0_rgb":
                for key, attr in mem_shape_meta.items():
                    example_obs_dict["mem"][key] = this_obs
...
def test():
    ...
    encoder = TransformerObsMemoryEncoder(
