...
from diffusion_policy.common.sampler import SequenceSamplerCowaAbs, get_val_mask
...
class CowaAbsDataset(BaseDataset):
    def __init__(self,
        shape_meta: dict,
        dataset_path: str,
        ...
        sampler = SequenceSamplerCowaAbs(
    ...
    def get_validation_dataset(self):
        ...
        val_set.sampler = SequenceSamplerCowaAbs(
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        pose_mat = pose_to_mat(np.concatenate([
            obs_dict[f'robot0_eef_pos'],
            obs_dict[f'robot0_eef_rot_axis_angle']
        ], axis=-1))
        action_mat = pose_to_mat(data['action'][...,0: 6])
        
        # convert pose to pos + rot6d representation
        obs_pose = mat_to_pose10d(pose_mat)
        action_pose = mat_to_pose10d(action_mat) # 3d xyz + 6d rot
    
        action_gripper = data['action'][..., 6:7]

        # generate data
        obs_dict[f'robot0_eef_pos'] = obs_pose[:,:3]
        obs_dict[f'robot0_eef_rot_axis_angle'] = obs_pose[:,3:]
    
        torch_data = {
            'obs': dict_apply(obs_dict, torch.from_numpy),
            'action': torch.from_numpy(np.concatenate([action_pose, action_gripper], axis=-1).astype(np.float32))
        }
        return torch_data
