...
from diffusion_policy.common.sampler_memory_moving import SequenceSampler, get_val_mask
...
class UmiMemoryDataset(BaseDataset):
    def __init__(self,
        shape_meta: dict,
        dataset_path: str,
        ...
        for key, attr in memory_shape_meta.items():
            if attr.get('type') == 'low_dim':  
                memory_keys.append(key)
                horizon = shape_meta['memory'][key]['horizon']
                key_horizon[key] = horizon
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.memory_keys:
            ...
            # T,H,W,C
            # convert uint8 image to float32
            memory_dict[key] = np.moveaxis(data[key], -1, 1).astype(np.float32) / 255.
            # T,C,H,W
            del data[key]
        ...
        torch_data = {
            ...
            'memory': dict_apply(memory_dict, torch.from_numpy),
